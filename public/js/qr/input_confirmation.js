...
$(document).ready(function() {
    ...
    function modalOpen() {
        ...
        if (processing_value === processing_sum) {
            ...
            camera_log("実績モーダル","表示",log_camera_url);
            ...
            camera_log("実績入力不正","入力した値が不正",log_camera_url);
            alert("入力した値が不正です。\n入力した値を確認してください");
        }
    }
    ...
    function modalClose() {
        ...
        $('.fix_value').remove();
        camera_log("実績モーダル","非表示",log_camera_url);
    }
...
//カメラが起動したlogを送る
function camera_log(location,status,log_camera_url)
{
    $.ajaxSetup({
        headers: {
